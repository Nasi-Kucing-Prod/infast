    ...
    try {
      ...
      if (Array.isArray(json.result)) {
        ...
        const filtered: WatchlistItem[] = json.result
          .filter((ticker: TickerDashboard) => watchlistSet.has(getUniqueId(ticker.currency_symbol, ticker.name)))
      ...
      {loading ? (
        ...
        <ul className="space-y-2">
          {watchlistItems.map((ticker, idx) => (
            <li key={ticker.uniqueId} className="flex justify-between items-center p-2 border rounded-md">
